...
# Version: 4.2 - Auto Image Detection + Windows RDP
# ============================================
...
setup_windows_rdp() {
    ...
    echo "  2) Use virt-manager for Windows VM (Recommended)"
...
setup_windows_virt_manager() {
    ...
    if ! command -v virt-manager &> /dev/null; then
        ...
        if [[ -f /etc/debian_version ]]; then
            ...
            dnf install -y virt-manager qemu-kvm libvirt
    ...
    case $rdp_choice in
        ...
        2)
            ...
            if [[ -f /etc/debian_version ]]; then
                ...
                dnf install -y remmina remmina-plugins-rdp
            fi
        ...
        4)
            ...
            if [[ -f /etc/debian_version ]]; then
                apt update
                apt install -y freerdp2-x11 remmina remmina-plugin-rdp rdesktop
            elif [[ -f /etc/redhat-release ]]; then
                dnf install -y freerdp remmina remmina-plugins-rdp rdesktop
            fi
...
check_installation() {
    ...
    if [[ $checks_passed -eq $total_checks ]]; then
        ...
        echo "1. If not in lxd group, run: usermod -aG lxd $USER"
        echo "2. If LXD not initialized, run: lxd init --auto"
        echo "3. If service not running: systemctl start snap.lxd.daemon"
...
check_system_ready() {
    if ! command -v lxc &> /dev/null; then
        ...
        echo "  1. Run: usermod -aG lxd $USER"
