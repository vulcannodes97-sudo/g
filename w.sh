#!/usr/bin/env bash
# ptero-license-manager.sh (webhook hardcoded)
...
db_update() {
  ...
  (
    flock -n 9 || { sleep 0.2; flock 9; }
    awk -F',' -v L="$lic" -v NL="$newline" 'NR==1{print;next}
      { if($1==L){print NL} else {print $0} }
...
cmd_generate() {
  ...
  discord_notify "New License Batch" "$body"
  printf "%s\n" "${arr[@]}"
}
...
cmd_validate() {
  ...
  if [ -z "$rec" ]; then
    echo "INVALID"
    discord_notify "Invalid Attempt" "License \`${lic}\` not found."
    return 2
  fi

  IFS=',' read -r _lic created assigned used meta <<<"$rec"
  local host
  ...
  if [ -z "$assigned" ]; then
    ...
    discord_notify "License Assigned" "License \`${lic}\` is now bound to \`${host}\`."
    echo "OK (assigned)"
  ...
  discord_notify "License Denied" "License \`${lic}\` belongs to \`${assigned}\` — attempted on \`${host}\`."
  echo "DENIED"
  return 3
}
...
cmd_revoke() {
  ...
  (
    flock -n 9 || { sleep 0.2; flock 9; }
    awk -F',' -v L="$lic" 'NR==1{print;next} $1!=L {print}' "$DB_FILE" > "$tmp" && mv "$tmp" "$DB_FILE"
  ) 9>"$LOCK_FILE"

  discord_notify "License Revoked" "License \`${lic}\` removed."
  echo "REVOKED"
}
